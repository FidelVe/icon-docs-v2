...
The execution environments create an abstraction layer between the three types of operators on ICON: [_smart contracts_](/icon-stack/smart-contracts), [_nodes (i.e. machines)_](/concepts/network), and [_users_](#level-5-end-user-applications-).

You can learn more about the execution environments in the [corresponding documentation section](/icon-stack/icon-execution-environments).
...
An application must connect to an ICON [node](/concepts/network/clients) in order to interact with the blockchain. This allows you to read from the blockchain and send transactions to be written on the blockchain.
...
You can use the [JSON-RPC API](/icon-stack/client-apis/json-rpc-api) to connect to the blockchain in order to read from it or write to it.

### Level 4: ICON client SDKs

There are a number of official and community-supported developer kits that allow you to access the ICON blockchain. These are referred to as the [_client standard developer kits (client SDKs)_](/icon-stack/client-apis#language-based). There are a number of different versions in different programming languages, for example [Javascript](/icon-stack/client-apis/javascript-sdk), [Python](/icon-stack/client-apis/python-sdk), and [Java](/icon-stack/client-apis/java-sdk).
