...
For a community member, staking refers to the act of supporting a delegate or delegate candidate by designating some amount of locked up [ICX](/concepts/economics/icx) for that candidate.
...
Staking to a delegate can be done through the [Hana wallet](https://icon.community/hanawallet/) or through the ICON JSON-RPC API directly using the [`setStake`](../../icon-stack/client-apis/json-rpc-api/#setstake) and [`setDelegation`](../../icon-stack/client-apis/json-rpc-api/#setdelegation) functions.
