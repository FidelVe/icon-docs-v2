...
Each ICON [transaction](../computational-utilities/transactions.md) requires computational resources to execute. Because this is a financial system, each transaction thus requires a fee. In the [ICON network](/concepts/network), these fees are referred to as STEP.

STEP fees are paid in [ICX](/concepts/economics/icx), the ICON network's native currency. The initial STEP price is approximately as follows:
