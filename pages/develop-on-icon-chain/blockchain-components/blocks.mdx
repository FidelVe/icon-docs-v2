...
Blocks are an efficient way to maintain a synchronized state between multiple machines. Instead of synchronizing every transaction individually, multiple transactions are synchronized in a single commit. This scheme of synchronization gives [validators](/operate-icon/how-to-run-a-validator-node) more time to form a consensus. This reduces the potential bottleneck that would be caused by network latency if each transaction were sent to all validator machines in individual messages.
