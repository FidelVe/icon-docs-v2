...
Smart contracts can be written and deployed by anyone as long as they have an ICON account, the funds to deploy the contract, and the ability to produce a contract coded in a valid [programming language](/icon-stack/smart-contracts/smart-contract-languages) for ICON.
...
Learn more about [composability](/icon-stack/smart-contracts/composability) in the corresponding section of the documentation.

### Limitations <a href="#limitations" id="limitations"></a>

As mentioned in the sections on the ICON execution environments (e.g. [Java](/icon-stack/icon-execution-environments/java)), there are certain features of a runtime environment that are used and expected by many programmers that are not available in ICON. This is by design, so that the blockchain can be deterministic and a consensus can be formed about the global state. Some limitations are as follows:
