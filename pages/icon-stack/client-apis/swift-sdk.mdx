...
`ICONService` is a class which provides APIs to communicate with ICON nodes. It enables you to easily use [ICON JSON-RPC APIs](json-rpc-api/v3) (version 3). All methods of `IconService` returns a `Request<T, ICError>` instance.
...
For more details of node URL and network id, see [ICON Network](/icon-stack/icon-networks/main-network) document.
...
* **error(error: Error)**
* **message(error: String**) - [JSON RPC Error Messages](/icon-stack/client-apis/json-rpc-api/v3#json-rpc-error-codes)
...
* [ICON Network](/icon-stack/icon-networks/main-network)
